...
-- 1.城市主题报表
USE ads_db;
CREATE TABLE IF NOT EXISTS ads_db.ads_city_report(
  ...
  growth_rate_daily DECIMAL(10,2) DEFAULT 0 COMMENT '日环比',
  -- 周环比
  pre_weekly_total_amount DECIMAL(18,2) DEFAULT 0 COMMENT '7日前销售额',
  growth_rate_weekly DECIMAL(10,2) DEFAULT 0 COMMENT '周环比',
  -- 月环比
  pre_monthly_total_amount DECIMAL(18,2) DEFAULT 0 COMMENT '30日前销售额',
  growth_rate_monthly DECIMAL(10,2) DEFAULT 0 COMMENT '月环比',
  -- 占比
  daily_prop DECIMAL(10,2) DEFAULT 0 COMMENT '占当日全平台比例（%）',
  PRIMARY KEY(city,daily_date)
  ) COMMENT '城市日度报表（含日/周/月环比）';