...
*优化效果：扫描行数减少 50%*  
 ...
 各层说明  
ODS 层：  
  ods_orders，原始订单数据，从业务库同步  
DWD 层：  
  dwd_orders_base：清洗后的订单明细  
  dwd_orders_wide：宽表（关联用户/商品维表）  
DWS 层：  
  dws_daily_summary：日汇总（GMV、订单量）  
  dws_weekly_summary：周汇总  
  dws_monthly_summary：月汇总  
  dws_city_summary：城市维度汇总  
  dws_category_summary：品类维度汇总  
  dws_user_summary：用户维度汇总  
ADS 层：  
  ads_city_report：城市报表（最终展示）  
  ads_user_report：用户报表（最终展示）  
  ads_category_report：品类报表（最终展示）  
